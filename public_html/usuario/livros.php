...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $usuario_id) {
    ...
    header("Location: livro.php?isbn=" . urlencode($livro['isbn'])); // ou '?codigo=' . urlencode($livro['codigo_interno'])
    exit;
}

// --- Livros relacionados (mesmo autor ou editora) ---
// Adicionado 'isbn' e 'capa_local' à seleção para consistência e links corretos
$rel_res = null; // Inicializa para garantir que a variável existe
$relacionados = $conn->prepare("SELECT id, titulo, capa_local, isbn FROM livros
                                WHERE id != ?
                                AND (autor_id = ? OR editora_id = ?) LIMIT 4");

if (!$relacionados) {
    // Apenas um log de erro ou mensagem para o admin, não interrompe a página
    error_log("Erro ao preparar consulta de livros relacionados: " . $conn->error);
} else {
    // Usar autor_id e editora_id para buscar relacionados, pois são IDs numéricos
...
<body>
    <?php // Supondo que seu header.php já inclua a navegação e o corpo HTML ?>
      ...
      <div class="row">
        ...
        <div class="col-md-8">
          ...
          <form method="POST" class="mt-3 d-flex gap-2">
            <?php if ($usuario_id): // Só mostra os botões se o usuário estiver logado ?>
